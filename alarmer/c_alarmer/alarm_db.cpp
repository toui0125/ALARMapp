...
:Robot(speak_color, error_speak_color)
{
  ...
  history_model = new HistoryModel("history.db");
}
...
int AlarmRobot::get_query_result_content
(void* data, int col_ctn, char** colval, char** colnm)
{
  return 
}
...
void AlarmRobot::set_all_time()
{
  ...
  std::string count_data_query = "select count(*) from history where START_ALARM_TIME = '" + string_start_alarm_time + "', STOP_ALARM_TIME = '" + string_stop_alarm_time + "', INTERVAL = '" + string_interval + "'";
  history_model->query_result = sqlite3_exec(history_model->db_conn, count_data_query.c_str(), 0, 0, &(history_model->db_err_msg));
